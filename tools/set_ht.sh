...
action=$1  # action should only be one of 'on', 'off', will do check anyway
...
then
	if [[ ${ht} == 1 ]]
	then
		for cpu_id in $(cat /sys/devices/system/cpu/cpu*/topology/thread_siblings_list | cut -s -d, -f2- | tr ',' '\n' | sort -un)
		do
